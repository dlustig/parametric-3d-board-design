...
import { collectSnapTargets, snapPoint, snapSegmentEnd } from '@/geometry/snap'
...
import type { DrawCursor, EditorState, Tool } from '../store.ts'
...
/** The subset of a PointerEvent the tool reads. */
...
function snapTargets(s: EditorState): SnapTargets {
  ...
  const targets = collectSnapTargets(s.project, currentContext(s), contextMatrix(s), [], findIntersections(expand(s.project)), s.gridMm)
  targetCache = { key: [s.project, s.editContext, s.gridMm], targets }
  return targets
}

/** SNAP_TOLERANCE_PX in the current context's mm (SPEC §4.6, §7.6). */
function toleranceMm(s: EditorState): number {
  return SNAP_TOLERANCE_PX / s.camera.zoom / scaleOf(contextMatrix(s))
}

function toContext(s: EditorState, svg: SVGSVGElement, e: DrawPointer): XY {
  return apply(invert(contextMatrix(s)), screenToWorld(svg, { x: e.clientX, y: e.clientY }))
}

function measure(from: XY | undefined, point: XY): Pick<DrawCursor, 'angleDeg' | 'lengthMm'> {
  if (from === undefined) return { angleDeg: 0, lengthMm: 0 }
  return { angleDeg: (Math.atan2(point.y - from.y, point.x - from.x) * 180) / Math.PI, lengthMm: dist(from, point) }
}

/** The snapped cursor for a raw context-space point. Alt held or Snap off: no snapping (SPEC §7.7). */
function cursorFor(s: EditorState, tool: DrawTool, raw: XY, altKey: boolean): DrawCursor {
  const placed = s.drawing?.points ?? []
  const from = placed[placed.length - 1]
  if (!s.snapEnabled || altKey) return { point: raw, guide: null, line: null, ...measure(from, raw) }
  const base = snapTargets(s)
  // The pending points are targets too, so the first point can be hit to close.
  const targets = { ...base, points: [...base.points, ...placed.slice(0, -1)] }
  const tol = toleranceMm(s)
  if (from === undefined || tool === 'rect') {
    const r = snapPoint(raw, targets, tol)
    return { ...r, ...measure(from, r.point) }
  }
  return snapSegmentEnd(from, raw, targets, tol, true)
}

function setDrawing(tool: DrawTool, points: XY[], cursor: DrawCursor | null): void {
 ...
 * with ≥ 3 points, one on the first point closes the polygon or Band.
 */
function placeTapped(tool: 'band' | 'polygon', cursor: DrawCursor): void {
  const s = useEditor.getState()
  ...
  if (first !== undefined && points.length >= 3 && dist(first, cursor.point) <= tol) {
    commitShape(tool, points, true)
    return
  }
...
export function drawPointerDown(svg: SVGSVGElement, e: DrawPointer, panning: boolean): void {
  const client = { x: e.clientX, y: e.clientY }
  down.set(e.pointerId, client)
  const s = useEditor.getState()
  if (!isDrawTool(s.tool)) return
  if (down.size > 1) {
...
export function drawPointerMove(svg: SVGSVGElement, e: DrawPointer): void {
  const client = { x: e.clientX, y: e.clientY }
  ...
  const s = useEditor.getState()
  if (!isDrawTool(s.tool) || down.size > 1) return
  const d = s.drawing
  if (s.tool === 'rect' && (d === null || press === null)) return // hover before a drag: nothing to show
  const cursor = cursorFor(s, s.tool, toContext(s, svg, e), e.altKey)
  setDrawing(s.tool, d?.points ?? [], cursor)
}

export function drawPointerUp(svg: SVGSVGElement, e: DrawPointer): void {
  down.delete(e.pointerId)
  const p = press
  if (p === null || p.id !== e.pointerId) return
  press = null
  const s = useEditor.getState()
  if (p.spoiled || !isDrawTool(s.tool)) return
  ...
  placeTapped(s.tool, cursorFor(s, s.tool, toContext(s, svg, e), e.altKey))
}

export function drawPointerCancel(e: DrawPointer): void {
  down.delete(e.pointerId)
