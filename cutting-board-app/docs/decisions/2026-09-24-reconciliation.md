...
9. **Compositing corrections after review.** The clipped re-paint patch draws only the crossed segment, is enlarged by a device-pixel amount in the editor (fixed mm in export) across the over Band's edges only (enlarging across both Bands' edges, as first decided, was withdrawn during Task 4; SPEC §16), and two further unsupported classes (`near-joint`, `occluded`) guarantee it never paints where it should not. Eligibility is decided per world intersection. See SPEC §5.2, §6.2, §16.
